<section class="inicio">

  <div class="general__titulo"><h1>Soluciones en tecnología para tu proyecto</h1></div>

  <div class="separacion"></div>

  <p class="parrafo__inicio">Nos preocupamos por dar siempre el mejor servicio a nuestros clientes logrando cumplir todas las expectativas</p>

  <div class="inicio__habilidades">

    <div class="habilidades__card">

      <div class="habilidades__img01"></div>
      <h2>Diseño responsivo</h2>
      <p>
        Creamos páginas web que se adaptan a cualquier tamaño de pantalla logrando que tu sitio web sea accesible desde cualquier dispositivo
      </p>

    </div>

    <div class="habilidades__card">

      <div class="habilidades__img02"></div>
      <h2>Diseño original</h2>
      <p>
        No ocupamos plantillas, cada diseño es único y personalizado, así podemos garantizar la originalidad de tu marca
      </p>

    </div>

    <div class="habilidades__card">
      <div class="habilidades__img03"></div>
      <h2>Pagos en linea</h2>
      <p>
        Alcanza un mayor mercado vendiendo en internet
      </p>
    </div>

    <div class="habilidades__card">

      <div class="habilidades__img04"></div>
      <h2>Disponibilidad</h2>
      <p>
        Aplicaciones web funcionales y estables disponibles en cualquier momento
      </p>

    </div>

  </div>

  <div class="general__titulo"><h1>Servicios destacados</h1></div>

  <div class="separacion"></div>

  <div class="inicio__destacados">

    <p>Servicios más solicitados por nuestros usuarios, tanto particulares como empresas</p>

    <div class="destacados__item">

      <a href="web" class="destacado__web">
        <div class="destacados__img01"></div>
        <h2>Diseño de páginas web</h2>
      </a>

      <a href="diseno" class="destacado__diseno">
        <div class="destacados__img02"></div>
        <h2>Diseño de aplicaciones web</h2>
      </a>

      <a href="ti" class="destacados__ti">
        <div class="destacados__img03"></div>
        <h2>Mantenimiento de equipos</h2>
      </a>

    </div>

  </div>


</section>

